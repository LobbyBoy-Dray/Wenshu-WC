...
class cpwsSpider:
	...
	def get_vjkl5(self, guid, number):
		session = self.session
		url = "http://wenshu.court.gov.cn/list/list/?sorttype=1&number=" + number + "&guid=" + guid + "&conditions=searchWord+QWJS+++" + parse.quote(self.keyword)
	...
	def main(self):
		countData = self.get_data(1)
		count = int(self.getWenShuNum(countData))
		return count
...
Spider1 = cpwsSpider(keyword)
count = Spider1.main()
addTime = time.strftime("%Y-%m-%d %H:%M:%S",time.localtime(time.time()))
with open('./文书ID/%s.txt' % filename, 'w', encoding = 'utf-8') as f:
	f.write(keyword)
	f.write('\n')
	f.write(str(count))
	f.write('\n')
	f.write(addTime)
	f.write('\n')
print("共%s条，时间：%s" %(str(count), addTime))
